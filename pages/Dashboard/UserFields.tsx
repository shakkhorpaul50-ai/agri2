...
const UserFields: React.FC<{ user: User }> = ({ user }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 min-h-screen bg-slate-50/30">
      <div className="flex justify-between items-center mb-10">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Agricare Intelligence Hub</h1>
          <p className="text-slate-500 text-sm mt-1">Cross-correlating real-time sensor data with biological crop requirements.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="flex justify-between items-center px-2">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Operational Plots</h3>
          </div>
          <div className="space-y-3 overflow-y-auto max-h-[75vh] pr-2 scrollbar-hide">
            {fields.map(f => (
              <button 
                key={f.field_id} 
                onClick={() => handleFieldSelect(f)} 
                className={`w-full text-left p-6 rounded-[2rem] border transition-all duration-300 transform ${
              ...
              >
                ...
                <div className="text-xs text-slate-400 font-medium">{f.location}</div>
                <div className="mt-4 flex items-center justify-between">
                  <span className="text-[9px] font-black uppercase text-slate-400 tracking-tighter bg-slate-50 px-2 py-1 rounded">{f.soil_type}</span>
                  <div className={`w-2 h-2 rounded-full ${selectedField?.field_id === f.field_id ? 'bg-emerald-500 animate-pulse' : 'bg-slate-200'}`}></div>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="lg:col-span-3">
          {!selectedField ? (
            <div className="bg-white rounded-[3rem] p-32 text-center border-2 border-dashed border-slate-200 flex flex-col items-center">
              <i className="fas fa-satellite-dish text-5xl text-slate-200 mb-6"></i>
              <h3 className="text-slate-400 font-bold text-xl">Select a plot to initialize the Agronomy AI.</h3>
            </div>
          ) : (
            <div className="space-y-8 animate-in fade-in zoom-in duration-500">
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-10">
                    ...
                    <div className={`px-4 py-2 rounded-2xl flex items-center gap-3 border ${
                      ...
                      'bg-red-500/10 border-red-500/20 text-red-400'
                       ...
                       <i className={`fas ${confidence === 'Full' ? 'fa-bolt' : 'fa-triangle-exclamation'}`}></i>
                       <span className="text-[10px] font-black uppercase tracking-widest">{confidence} AI Confidence</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      ...
                      <div key={i} className={`bg-white/5 border border-white/10 p-5 rounded-3xl flex items-center gap-4 backdrop-blur-md transition-all ${p.active ? 'opacity-100 scale-100' : 'opacity-20 scale-95 grayscale'}`}>
              ...
              {loading ? (
                <div className="bg-white p-32 text-center rounded-[3rem] border border-slate-100 shadow-sm flex flex-col items-center">
                  ...
                  <h3 className="text-2xl font-black text-slate-800 tracking-tight">Correlating Biological Markers...</h3>
                  <p className="text-slate-400 mt-2">The Gemini AI is currently matching your sensors to optimal harvest variants.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  {/* Strategy Column */}
                  <div className="lg:col-span-2 space-y-8">
                    {/* Soil Restoration Insight */}
                    <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm group relative overflow-hidden">
                      <div className="absolute top-0 right-0 p-8 text-emerald-500/5 text-9xl group-hover:scale-110 transition-transform">
                        <i className="fas fa-flask"></i>
                      </div>
                      <h3 className="font-black text-2xl text-slate-900 mb-8 flex items-center gap-3">
                        <i className="fas fa-leaf text-emerald-600"></i> Biological Soil Profile
                      </h3>
                      <div className="space-y-6 relative z-10">
                        <div className="p-8 rounded-[2.5rem] bg-emerald-50/30 text-slate-700 border border-emerald-50">
                          <p className="text-lg leading-relaxed font-bold italic text-slate-800">
                            "{soilInsight?.summary || "Sensor handshake required to analyze biological restoration path."}"
                          </p>
                        </div>
                        <div className="bg-slate-900 p-6 rounded-[2.5rem] flex items-center justify-between shadow-2xl">
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-emerald-500 rounded-2xl flex items-center justify-center text-white shadow-lg">
                              <i className="fas fa-prescription-bottle-medical"></i>
                            </div>
                            <div>
                              <div className="text-[10px] font-black uppercase text-emerald-400 tracking-widest">Target Treatment</div>
                              <div className="font-bold text-white text-lg">{soilInsight?.soil_fertilizer || "Awaiting Hardware Connection"}</div>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between items-end mb-8 px-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Weighted Sensor Correlation</span>
                      </div>
                        ...
                        {recommendations && recommendations.length > 0 ? recommendations.map((r, i) => (
                          <div key={i} className="bg-white p-8 rounded-[3.5rem] border border-slate-100 transition-all hover:shadow-2xl hover:-translate-y-1 group">
                            <div className="flex justify-between items-start mb-8">
                              <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-[2rem] flex items-center justify-center shadow-inner group-hover:bg-emerald-600 group-hover:text-white transition-all duration-500">
                                <i className={`fas ${r.icon || 'fa-seedling'} text-2xl`}></i>
                              </div>
                              <div className="text-right">
                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Match Rating</div>
                                <div className={`text-4xl font-black ${r.suitability > 75 ? 'text-emerald-600' : 'text-slate-900'}`}>{r.suitability}%</div>
                              </div>
                            </div>
                            ...
                            <div className="w-full h-2 bg-slate-100 rounded-full mb-6 overflow-hidden">
                              <div 
                                className={`h-full rounded-full transition-all duration-1000 ${r.suitability > 75 ? 'bg-emerald-500' : 'bg-slate-400'}`} 
                            ...
                            <div className="space-y-4">
                              <div className="bg-slate-900 p-5 rounded-[2rem] shadow-lg">
                                <div className="text-[10px] font-black text-emerald-400 uppercase mb-2">Fertilization Logic</div>
                              ...
                              <div className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-50">
                                <div className="text-[9px] font-black text-emerald-700 uppercase mb-1 tracking-widest">AI Reasoning</div>
                          ...
                          <div className="col-span-full py-24 bg-white rounded-[3rem] border border-dashed border-slate-200 text-center flex flex-col items-center">
                             <i className="fas fa-microscope text-4xl text-slate-200 mb-4 animate-pulse"></i>
                             <p className="text-slate-400 font-bold uppercase text-xs tracking-widest">Awaiting Harvest Correlation Data...</p>
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="space-y-12">
                        {managementPlan && managementPlan.length > 0 ? (
                          ...
                          <div className="text-center py-20 opacity-30">
                             ...
                             <p className="font-bold text-xs uppercase tracking-widest">No Active Tasks</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="mt-12 p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100">
                         <div className="flex gap-4 items-start">
                            ...
                            <p className="text-[10px] text-slate-400 font-medium leading-relaxed uppercase tracking-widest">
                              Roadmap updates dynamically via LoRaWAN node heartbeat.
                            </p>
                         </div>
                      </div>
                    </div>
      ...
      {showAddFieldModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] w-full max-w-md p-10 shadow-2xl animate-in zoom-in duration-300">
            <h2 className="text-3xl font-black mb-8 text-slate-900">Deploy Plot Hub</h2>
            <form onSubmit={handleAddField} className="space-y-6">
              <div>
                <label className="block text-xs font-black text-slate-400 uppercase mb-2 ml-1">Plot Alias</label>
                <input required type="text" value={newFieldData.name} onChange={e => setNewFieldData({...newFieldData, name: e.target.value})} className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none outline-none focus:ring-2 focus:ring-emerald-500 font-medium" placeholder="Paddy Sector Alpha" />
              </div>
              ...
              <button type="submit" className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl shadow-emerald-200 hover:bg-emerald-700 transition-all">Register Node</button>
              <button type="button" onClick={() => setShowAddFieldModal(false)} className="w-full py-2 text-slate-400 font-bold text-xs uppercase hover:text-slate-600 mt-2">Cancel Deployment</button>
            </form>
          </div>
        </div>
      )}
