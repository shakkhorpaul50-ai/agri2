...
import { syncFields } from '../../services/db';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button className="text-emerald-600 font-bold text-sm hover:underline">View All Details</button>
      </div>
          ...
          {fields.map(f => (
            <div key={f.field_id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
              ...
              <div className="pt-6 border-t border-slate-50 flex justify-between items-center">
                ...
                <div className="text-[10px] text-emerald-600 font-black uppercase tracking-widest">Active Monitoring</div>
              </div>
            </div>
          ))}
          {fields.length === 0 && (
            <div className="col-span-full py-20 bg-slate-50 rounded-[3rem] border border-dashed border-slate-200 text-center text-slate-400">
              ...
              <p className="text-sm">Head over to the "My Fields" tab to add your first plot.</p>
            </div>
          )}
        </div>
      )}
