...
export const getCropAnalysis = async (field: Field, latestData: SensorData) => {
  // Cloudflare Pages will inject the API_KEY from your environment variables
  const apiKey = process.env.API_KEY;
  
  if (!apiKey) {
    console.error("API_KEY is missing. Please set it in your Cloudflare Pages environment variables.");
    return null;
  }
